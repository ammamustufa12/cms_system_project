...
@section('content')
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                        <h4 class="mb-sm-0">Edit {{ $contentType->name }} Content</h4>

                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);">Dashboard</a></li>
                                <li class="breadcrumb-item"><a href="{{ route('content-types.index') }}">Content Types</a></li>
                                <li class="breadcrumb-item"><a href="{{ route('content-types.content-items.index', $contentType->slug) }}">Content Items</a></li>
                    ...
                    <!-- Form -->
                    <form action="{{ route('content-types.content-items.update', [$contentType->slug, $contentItem->id]) }}" method="POST"
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="mt-2">
                                            <span class="badge bg-{{ $contentItem->status === 'published' ? 'success' : ($contentItem->status === 'draft' ? 'warning' : 'secondary') }}">
                                                {{ ucfirst($contentItem->status) }}
                                            </span>
                                            @if($contentItem->published_at)
                                                <span class="badge bg-info">
                                                    Published: {{ $contentItem->published_at->format('M d, Y H:i') }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div id="dynamicFields">
                                            @foreach ($fieldsSchema as $fieldKey => $field)
                                                <div class="dynamic-field" data-field-type="{{ $field['type'] }}">
                                                    @php
                                                        $fieldValue = old($fieldKey, $contentItem->field_data[$fieldKey] ?? null);
                                                    @endphp
                                                    {!! App\Services\FormRendererService::renderField($fieldKey, $field, $fieldValue, $errors) !!}
                                                </div>
                                            @endforeach
                                            ...
                                            <div class="dynamic-field" data-field-type="checkbox">
                                                <label>
                                                    <input type="checkbox" name="published_at" value="1"
                                                        {{ old('published_at', $contentItem->status === 'published' ? '1' : '') ? 'checked' : '' }}>
                                                    Publish
                                                </label>
                                                @error('published_at')
                                ...
                                <div class="card mt-3">
                                    <div class="card-header">
                                        <h6 class="card-title mb-0">
                                            <i class="fas fa-history me-2"></i>Content History
                                        </h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <div>{{ $contentItem->created_at->format('M d, Y H:i') }}</div>
                                                @if($contentItem->createdBy)
                                                    <small class="text-muted">by {{ $contentItem->createdBy->name }}</small>
                                                @endif
                                            </div>
                                            <div class="col-md-6">
                                                <small class="text-muted">Last Updated:</small>
                                                <div>{{ $contentItem->updated_at->format('M d, Y H:i') }}</div>
                                                @if($contentItem->updatedBy)
                                                    <small class="text-muted">by {{ $contentItem->updatedBy->name }}</small>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Mark form as saved initially
            document.getElementById('contentForm').dataset.saved = 'true';
            
            // Auto-generate slug from title (but don't override existing slug unless empty)
            ...
            titleInput.addEventListener('input', function() {
                ...
                document.getElementById('contentForm').dataset.saved = 'false';
            });
            ...
            function generatePreview() {
                ...
                // Status badge
                const isPublished = formData.get('published_at');
                const statusBadge = isPublished ? 
                    '<span class="badge bg-success">Published</span>' : 
                    '<span class="badge bg-warning">Draft</span>';
                previewHtml += '<div class="mb-4">' + statusBadge + '</div>';
                        ...
                        switch (field.type) {
                            ...
                            case 'image':
                                ...
                                if (fileInput && fileInput.files[0]) {
                                    ...
                                    const existingValue = @json($contentItem->field_data);
                                    if (existingValue[fieldKey]) {
                                        previewHtml += '<p class="text-muted">Current file: ' + 
                                            existingValue[fieldKey] + '</p>';
                                    }
                                }
                                break;
                            case 'boolean':
                            ...
                            case 'datetime':
                                ...
                                break;
                            case 'textarea':
                                previewHtml += '<div class="border p-3 rounded" style="white-space: pre-wrap;">' +
                                    value + '</div>';
                                break;
                            default:
            ...
            function autoSave() {
                ...
                autoSaveTimer = setTimeout(function() {
                    ...
                    // Only auto-save if title is present and form has changes
                    if (formData.get('title') && form.dataset.saved === 'false') {
                        fetch(form.action, {
                                method: 'POST',
                                body: formData,
                                headers: {
            ...
            // Start auto-save when user makes changes
            ...
            fileInputs.forEach(function(input) {
                input.addEventListener('change', function() {
                    ...
                    if (files.length > 0) {
                        ...
                        infoDiv.innerHTML = `<small class="text-muted">Selected: ${fileInfo}</small>`;
                    }
                });
            });
        ...
        // Form submission confirmation for unsaved changes
        window.addEventListener('beforeunload', function(e) {
            const form = document.getElementById('contentForm');
            
            // Check if form has unsaved changes
            if (form.dataset.saved === 'false') {
                e.preventDefault();
